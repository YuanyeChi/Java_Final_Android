...
public class ProblemHelper {
    ...
    public static boolean[] judgeSelectAnswer(String[] myAnswer, TotalProblems problem) {
        ...
        if (problem.getProblemType().equals("Reading") || problem.getProblemType().equals("Cloze")) {
            ...
            if (problem.getAnswer().contains(";")) {
                Log.i("CYY", "含;");
                res[0] = problem.getAnswer().toLowerCase().equals(myAnswer[0].toLowerCase());
                return res;
            } else if (problem.getAnswer().contains("/")) {
                String[] temp1 = problem.getAnswer().split("/");
                for (int j = 0; j < temp1.length; j++) {
                    res[0] = temp1[j].toLowerCase().equals(myAnswer[0].toLowerCase());
    ...
    public static void store(TotalProblems problem, SQLiteDB recordDB, boolean[] result) {
        //写作次数加一既可
        if (problem.getProblemType().equals("Writing")) {
...
//        }
        ...
        for (int i = 0; i < list.size(); i++) {
            if (thisScore == 1) {
                ...
                totalNum = 1;
                break;
            } else if (list.get(i).getFinishedTimes() != 0 && list.get(i).getProblemType().equals(problems.getProblemType()) && list.get(i).getScore() < thisScore) {
                beatNum += 1;
                totalNum += 1;
            } else if (list.get(i).getFinishedTimes() != 0 && list.get(i).getProblemType().equals(problems.getProblemType()) && list.get(i).getScore() >= thisScore) {
                totalNum += 1;
        ...
        for (int i = 0; i < list.size(); i++) {
            if (thisScore == 1) {
                ...
                totalNum = 1;
                break;
            } else if (list.get(i).getFinishedTimes() != 0 && list.get(i).getScore() < thisScore) {
                beatNum += 1;
                totalNum += 1;
            } else if (list.get(i).getFinishedTimes() != 0 && list.get(i).getScore() >= thisScore) {
                totalNum += 1;
        ...
        if (record.getFinishedTimes() == 0 && thisScore < 0.5)
            report += "<p>第一次做，分数稍微低了点，继续努力！</p>";
        else if (record.getFinishedTimes() == 0 && thisScore >= 0.5)
            report += "<p>第一次做就那么厉害，做的不错！</p>";
        else if (record.getScore() != 0 && record.getScore() > thisScore)
            report += "<p>相较上次这次退步了，要注意！</p>";
        else if (record.getScore() != 0 && record.getScore() <= thisScore)
            report += "<p>有进步，再接再厉！</p>";
